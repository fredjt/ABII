//
// Created by Trent Tanchin on 7/31/25.
//

#ifndef ABII_LOGGER_H
#define ABII_LOGGER_H

#include <fstream>
#include <sys/stat.h>

extern inline const std::string abii_logdir = getenv("ABII_LOGDIR") ? getenv("ABII_LOGDIR") : "/tmp/abii/abii_log";

class Logger
{
    static thread_local std::ofstream ofstream_;
    const char* scope_;

public:
    Logger() = delete;

    explicit Logger(const char* scope) : scope_(scope)
    {
        mkdir(abii_logdir.c_str(), 0775);
        if (!ofstream_.is_open())
            ofstream_ = std::ofstream(abii_logdir + "/###TRACE_LOG###.log", std::ios::app);

        ofstream_ << "Entering " << scope_ << std::endl;
    }

    ~Logger() { ofstream_ << "Exiting " << scope_ << std::endl; }
    friend void LoggerInit();
};

#endif //ABII_LOGGER_H
